...
function MeerStack-Configuration {
    ...
    try {
        ...
        if ($reader.Read()) {
            ...
            $registryKeysToCheck = Get-ReaderValue -Reader $reader -Column "RegistryKeysToCheck" -Default ""
            $config.Checks["RegistryKeys"] = @{
                Enabled                 = ((Get-ReaderValue -Reader $reader -Column "RegistryKeys" -Default $false) -eq $true)
                Interval                = [int](Get-ReaderValue -Reader $reader -Column "RegistryKeysInterval" -Default 3600)           # 60-minutes
                RegistryKeysToCheck     = if ($registryKeysToCheck) {
                    try { $registryKeysToCheck | ConvertFrom-Json } catch { @() }
                } else {
                    @()
                }
            }
